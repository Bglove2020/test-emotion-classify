...
#定义正则表达式，并生成模式，^表示取反，+表示匹配1-∞多个，【】中的写法可以匹配汉字，英文，空格，数字，常见中英文符号。
pattern = re.compile(r'[^\u4e00-\u9fa5\d\w\s\u3002\uff1b\uff0c\uff1a\u201c\u201d\uff08\uff09\u3001\uff1f\u300a\u300b\u3010\u2014\u3011\u2022\u300c\u300d!#$%&\'\"()*+,-.·/:;<=>?@\[\\\]\^_`{|}~]+')
for line in lines:
    ll = re.split(r'\s+', line.lstrip())
    if pattern.search(ll[0])!=None:
        continue
    data_txt.append(ll[0])
    data_label.append(ll[1])
print("剔除数据前数据量：",len(lines),"剔除颜文字后数据量：",len(data_label),"删除数据量：",len(lines)-len(data_label))

label_dict={"sadness":1,"happiness":2,"like":3,"anger":4,"fear":5,"surprise":6,"disgust":0}
...
class TCN(nn.Module):
    def __init__(self, max_dict_index,input_size, num_channels, kernel_size, dropout):
        ...
        self.emb = nn.Embedding(max_dict_index, 256)
...
class TCN_LSTM(nn.Module):
    def __init__(self, max_dict_index,input_size, output_size, num_channels, kernel_size, dropout):
        ...
        self.lstm =mylstm(128,64,1,output_size,0.0)

    def forward(self, x):
        # x的形状为(batch_size, seq_len, num_features)
        x= self.tcn(x)
        x = x.permute(0, 2, 1)  # 将输入的最后两个维度交换，变成(batch_size, seq_len, num_features)
        x=self.lstm(x)
        # 输出的形状为(batch_size, output_size)
        return x
# 构建 TCN 模型
batch_size = 32
max_dict_index =23255

train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True)
model = TCN_LSTM(max_dict_index, input_size=256, output_size=7, num_channels=[128, 128, 128, 128, 128, 128], kernel_size=3, dropout=0.1)
